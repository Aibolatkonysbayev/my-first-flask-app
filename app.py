...
app.config['SECRET_KEY'] = 'th5_nht2h_88ijh,_e_123dhd,_75mm_vvv_34m9men_.[ds8' # >>> ОБЯЗАТЕЛЬНО ЗАМЕНИ НА СВОЮ <<<
...
class User(db.Model, UserMixin):
    ...
    # --- Конец Связи с вакансиями (Шаг 21) ---
...
# --- НАЧАЛО НОВОГО КОДА: Маршрут Создания Вакансии (Шаг 22) ---
# @app.route('/jobs') # TODO: СЮДА ПОЗЖЕ МАРШРУТ ДЛЯ СПИСКА ВАКАНСИЙ
# @login_required
# def list_jobs():
#    pass # TODO: Реализация списка вакансий

@app.route('/jobs/create', methods=['GET', 'POST'])
@login_required # Только авторизованные пользователи могут создавать вакансии
def create_job():
    if request.method == 'POST':
        ...
        if not title or not description:
            ...
            # Передаем обратно на форму создания вакансии
            return render_template('create_job.html', user=current_user) # Передаем user для шаблона

        # Создаем новый объект Vacancy
        new_job = Vacancy(
            title=title,
            description=description,
            # !!! САМОЕ ВАЖНОЕ: Связываем вакансию с текущим вошедшим пользователем !!!
            user_id=current_user.id # current_user - это объект пользователя, вошедшего в систему
        )

        # Добавляем вакансию в сессию базы данных и сохраняем
        db.session.add(new_job)
        db.session.commit()

        flash('Вакансия успешно создана!', 'success')
        # TODO: Перенаправить на страницу списка вакансий после создания
        return redirect(url_for('index')) # Временно перенаправляем на главную

    # Отображение формы создания вакансии при GET-запросе
    return render_template('create_job.html', user=current_user)
# --- КОНЕЦ НОВОГО КОДА: Маршрут Создания Вакансии (Шаг 22) ---
